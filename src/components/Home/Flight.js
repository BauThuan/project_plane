...
import { RoundTrip, OneWay, ManyCities, isTitle } from "../Redux/Action";
...
const Flight = () => {
  ...
  const handleShowModalSelectPeople = (type) => {
  ...
  const handleSearchRoundWay = (type) => {
    if (listPlaneFlight.length <= 0) {
      if (profile.length <= 0) {
        toast.error(
          languageEN
            ? "You need to update your personal information before registering for a flight!"
            : "Bạn cần cập nhật thông tin cá nhân rồi mới đăng ký được chuyến bay !"
        );
      } else {
        if (type === "Khứ hồi") {
          let {
              ...
              <div className="flight--type--title">
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => handleShowModalSelectPeople(checkRadio)}
                  >
                    {languageEN
